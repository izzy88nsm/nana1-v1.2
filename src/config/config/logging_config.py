...
def get_logger(
    ...
    if enable_console and logging.StreamHandler not in existing_handlers:
        ...
        console_formatter = console_formatter or logging.Formatter(
    ...
    if enable_file:
        ...
        if not any(isinstance(h, type(file_handler)) for h in logger.handlers):
            file_formatter = file_formatter or logging.Formatter(
    ...
    # Set effective logger level based on handlers
